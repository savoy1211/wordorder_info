...
train = ModulateText(mega_text_90, language="english")
model = NgramModel(train.tokens, alpha=0.1, n=3)
print("LM setup complete!")
test = ModulateText(mega_text_10, language="english")
print("Test text created!")
t = LMResults(model, test)
t.get_results("LM_n3_(ENGLISH)_a0.1_OI_train(90)_test(10).txt")
file = open("LM_n3_(ENGLISH)_a0.1_OI_train(100)_test(100)", "wb")
p.dump(model, file)
print("LM created!")

# Get Results (Train Random Outbound 90, Test Random Inbound 10)
train = ModulateText(mega_text_90, language="english", randomize_across_sentence=True)
model = NgramModel(train.tokens, alpha=0.1, n=3)
file = open("LM_n3_(ENGLISH)_a0.1_R_train(90_o)_test(10_i)", "wb")
p.dump(model, file)
print("LM setup complete!")
test = ModulateText(mega_text_10, language="english", randomize_within_sentence=True)
print("Test text created!")
t = LMResults(model, test)
